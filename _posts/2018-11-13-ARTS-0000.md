...
title      ARTS_002
subtitle    week 02
date       2018-11-11 120000
author     Xion
header-img imgarts.jpg
catalog true
tags
    - arts
    - 算法
...
httpsleetcode.comproblemsprint-binary-tree
...
public class Solution {
    public ListListString printTree(TreeNode root) {
        int height = getHeight(root);
        String[][] res = new String[height][(1  height) - 1];
        for(String[] arrres)
            Arrays.fill(arr,);
        ListListString ans = new ArrayList();
...
利用递归的思路写的内容相对代码量比较小，负责度为 O(2h)，h是树的高度
...
[A Tour of Reinforcement Learning The View from Continuous Control](chrome-extensionikhdkkncnoglghljlkmcimlnlhkeamadpdf-viewerwebviewer.htmlfile=https%3A%2F%2Farxiv.org%2Fpdf%2F1806.09460.pdf)
...
subject to $x_{t+1} = hatvarphi(x_t,u_t) + w_t,u_t = pi_t(T_t)$
...
如果直接优化$p(z)$是比较苦难的，所以我们可以考虑由参数向量参数化的族$thetasymp(u;thetasym)$

$maximize_thetasym E_p(z;thetasym)[R(z)]$
...
其中$J(thetasym)=E_p(z;thetasym)[R(z)]$
...
就是利用梯度下降的方法来进行搜索，对$thetasym$来进行有方向的更新。
...
下周看看下相关的背景资料
httpsgithub.comwwxFromTjuawesome-reinforcement-learning-zh
...
hello
   ...
   ncalls  tottime  percall  cumtime  percall filenamelineno(function)
        1    0.000    0.000    0.000    0.000 string1(module)
        1    0.000    0.000    0.000    0.000 haha.py1(test)
...
[聪明的员工为什么那不愿多干活多拿奖金](httpsmp.weixin.qq.coms__biz=MzA4NTU2MTg3MQ==&mid=2655165926&idx=1&sn=071a70e23ed3372a236c293c278d4752&chksm=84603a86b317b390e0388d10d3582b9c0aa4d9f499b0a80455b74f49ed551dd67204afbff052&mpshare=1&scene=1&srcid=1112XfEPX1keklxOrv11TV3a&pass_ticket=%2BvgahYeaUrXd4sn7uzhC%2BjghpjyaJAn7920sGay3U9fBKffxf%2FngTwLbAztTYl4%2B#rd)
